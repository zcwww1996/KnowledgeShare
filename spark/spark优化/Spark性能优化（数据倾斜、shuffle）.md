...
参考文献<br>
[Spark性能优化指南——基础篇](https://zhuanlan.zhihu.com/p/21922826 "Spark性能优化指南——基础篇")<br>
[Spark性能优化指南——高级篇](https://zhuanlan.zhihu.com/p/22024169 "Spark性能优化指南——高级篇")<br>
[Spark Shuffle原理、Shuffle操作问题解决和参数调优](https://www.cnblogs.com/arachis/p/Spark_Shuffle.html "Spark Shuffle原理、Shuffle操作问题解决和参数调优")
...
![数据倾斜例子](http://img.hb.aicdn.com/ea3d7d8d1542b7a4bdaef1105f2e4f4d4d6504712abb4-OSxOD2_fw658 "数据倾斜例子")
## 如何定位导致数据倾斜的代码
...
![sparkUI—数据倾斜](http://img.hb.aicdn.com/6d7247c810e63340a030dfbb81ac2d45388f9fdd60859-jnRh0U_fw658 "sparkUI—数据倾斜")

接着我们就需要根据stage划分原理，推算出来发生倾斜的那个stage对应代码中的哪一部分，这部分代码中肯定会有一个**shuffle类算子**。精准推算stage与代码的对应关系，需要对Spark的源码有深入的理解，这里我们可以介绍一个相对简单实用的推算方法：*只要看到Spark代码中出现了一个shuffle类算子或者是Spark SQL的SQL语句中出现了会导致shuffle的语句（比如group by语句），那么就可以判定，以那个地方为界限划分出了前后两个stag*e。
...
1. stage1，主要是执行从reduceByKey到collect操作，stage1的各个task一开始运行，就会首先执行shuffle read操作。执行shuffle read操作的task，会从stage0的各个task所在节点拉取属于自己处理的那些key，然后对同一个key进行全局性的聚合或join等操作，在这里就是对key的value值进行累加。stage1在执行完reduceByKey算子之后，就计算出了最终的wordCounts RDD，然后会执行collect算子，将所有数据拉取到Driver上，供我们遍历和打印输出。
...
知道了数据倾斜发生在哪里之后，通常需要分析一下那个执行了shuffle操作并且导致了数据倾斜的RDD/Hive表，查看一下其中key的分布情况。这主要是为之后选择哪一种技术方案提供依据。
...
![提高并行度](http://img.hb.aicdn.com/3f987a55f9c3547642a60e64458b2392a5619d00258c7-uk3aIe_fw658 "提高并行度")
...
![两阶段聚合](http://img.hb.aicdn.com/cf696aabafb869106585cc3a7782b010cb2f9e50309c6-mdLYsN_fw658 "两阶段聚合")
...
![reduce转map](http://img.hb.aicdn.com/c208ee811c6a1ffc3f853e67697b3e67b1f5b6db1ed19-FzDvJh_fw658 "reduce转map")
...
![采样倾斜key并分拆join操作](http://img.hb.aicdn.com/8adc714ca5fc587b0668410f705281dbcc9349de43e23-WyhvpZ_fw658 "采样倾斜key并分拆join操作")
...
Scala版
```scala
  def solution2(pairs1: RDD[(Int, String)], pairs2: RDD[(Int, String)]): RDD[(Int, (String, String))] = {
    val SPLIT = "_"
    val size = 100
    // 首先，将其中一个key分布相对较为均匀的RDD膨胀100倍。
    val expandedRDD = pairs1.flatMap(t => {
      // multi 100
      val array = new Array[(String, String)](size)
      for (i <- 0 until size) {
        array(i) = (i + SPLIT + t._1, t._2.toString)
    ...
    pairs2.map(t => {
      val prefix = new Random().nextInt(size)
      (prefix + SPLIT + t._1, t._2) //key random hash
    }).join(expandedRDD).map(t => {
      val key = t._1.split(SPLIT)(1).toInt
      (key, t._2)
    })
...
如果要处理一个较为复杂的数据倾斜场景，那么可能需要将多种方案组合起来使用。比如说，我们针对出现了多个数据倾斜环节的Spark作业，可以先运用解决方案一和二，预处理一部分数据，并过滤一部分数据来缓解；其次可以对某些shuffle操作提升并行度，优化其性能；最后还可以针对不同的聚合或join操作，选择一种方案来优化其性能。

# shuffle调优

参考：[Spark shuffle的模式详细过程](https://www.cnblogs.com/yangsy0915/p/6058611.html)


**<font color=red>影响一个Spark作业性能的因素，主要还是代码开发、资源参数以及数据倾斜</font>**，shuffle调优只能在整个Spark的性能调优中占到一小部分而已
...
![未经优化的HashShuffleManager](http://img.hb.aicdn.com/3b8df9f9405d6fbaa9a26495086ab3784f56c07156e94-GhtaeE_fw658 "未经优化的HashShuffleManager")
...
![优化后的HashShuffleManager](http://img.hb.aicdn.com/4ace52a8661ae44ae3e0f0bdebf074026b27d87e4b044-VJk8xP_fw658 "优化后的HashShuffleManager")
...
![SortShuffleManager—普通](http://img.hb.aicdn.com/2d00db401baa98fc5e198d4717ca22c5cf3a6d39373a9-Sxbkhe_fw658 "SortShuffleManager—普通")
...
![ SortShuffleManager—bypass运行机制](http://img.hb.aicdn.com/baff3662beace02b2c2bb3cd5f29c66e16d34bc93d469-LhsQgx_fw658 " SortShuffleManager—bypass运行机制")
...
![shuffle算子join-1](http://img.hb.aicdn.com/59a9c03e06d21be68bd20c5f8c5c3210858827e4339f-6EXqbX_fw658 "shuffle算子join-1")
![shuffle算子join-2](http://img.hb.aicdn.com/e2ecff294bf9c2f2d15a5356ffba941eda1f00715f50-QuQPJQ_fw658 "shuffle算子join-2")
