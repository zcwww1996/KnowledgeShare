...
**函数和方法区别**<br>
1) 方法只能用def接收，函数可以用def接收，也可以用val接收。
2) 当函数用def来接收之后，不再显示为function，转换为方法
3) **方法可以省略参数，函数不可以。==函数可以作为方法的参数==**。
...
//测试
println(mulByOneTime(6, 7))
println(mulByOneTime1(6)(7))
println(mulByOneTime2(6)(7))
```
